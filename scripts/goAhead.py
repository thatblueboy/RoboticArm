...
import roslib
from math import tan,acos,asin,cos,sin
...
class goAhead_Server:
  def __init__(self,cmd_vel):
    self.server = actionlib.SimpleActionServer('go_Ahead', goAheadAction, self.execute, False)
    self.server.start()
    self.theta1=0
    self.theta2=0
    self.l1=30
...
#   def execute(self, goal):
    ...
    # BROTHER WHY DONT YOU HAVE AUTOSAVE
      # FK
      ...
      if self.InitError2>=0:
        target_theta2=acos(((x*self.x)+(y*y)-(self.l2*self.l2)-(self.l1*self.l1))/(2*self.l1*self.l2))
        target_theta1=atan(y/x)-atan(self.l2*sin(theta2)/(self.l1+self.l2*cos(theta2)))
      else:
        target_theta2=-1*acos(((x*self.x)+(y*y)-(self.l2*self.l2)-(self.l1*self.l1))/(2*self.l1*self.l2))
        target_theta1=atan(y/x)+atan(self.l2*sin(theta2)/(self.l1+self.l2*cos(theta2)))
